...
## PENTING: HuggingFace Ganti Domain (Juli 2026)

HuggingFace **mematikan total** domain lama `api-inference.huggingface.co` per akhir 2025,
pindah ke sistem baru **"Inference Providers"** di domain `router.huggingface.co`, dengan format
request yang sekarang **persis kayak OpenAI** (`/v1/chat/completions` + array `messages`),
bukan format `inputs` prompt gabungan manual lagi. Ini udah saya update di kode dan `.env`.

Kalau kamu nemu error `getaddrinfo ENOTFOUND api-inference.huggingface.co` di log Railway,
itu tandanya masih pakai domain lama — pastikan `BASE_URL` di environment variable kamu:
```
